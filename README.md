...
如果你要我一键发布到 GitHub（自动补 checkpoint 日志 + 提交 + 推送 + Draft PR），直接说：
...
```text
发布一下：UI 原型输出可控性规范 | 增加 PRD UI 契约、风格守门和质量门禁
```
...
| 2026-05-18 | 新增 `scripts/publish-github.sh` 一键发布脚本；支持自动写 `knowledge/codex-checkpoints.md`、提交、推送、Draft PR | 把“发布到 GitHub + 更新日志”从手工流程改为单指令流程 |
